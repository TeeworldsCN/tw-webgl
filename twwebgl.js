...
tw.sTexCoords = function (state) {
    if (state) tw.gl.enableVertexAttribArray(tw.stdShader.vTexCoordAttr);
    else tw.gl.disableVertexAttribArray(tw.stdShader.vTexCoordAttr);

    tw.gl.uniform1i(tw.stdShader.uTexCoord, state);
};

tw.sVertexColor = function (state) {
    if (state) tw.gl.enableVertexAttribArray(tw.stdShader.vVertexColor);
    else tw.gl.disableVertexAttribArray(tw.stdShader.vVertexColor);
...
tw.getParams = function () {
    ...
    nvPairs.forEach(function (pair) {
        ...
        if (!(n in o)) o[n] = v;
        else if (o[n] instanceof Array) o[n].push(v);
        else o[n] = [o[n], v];
    });
...
tw.Map = function (mapData) {
    ...
    for (var i = 0; i < imgsInfo.num; i++) {
        ...
        if (imgInfo.external) {
            ...
            if (tex != undefined) this.textures.push(tex);
        } else {
            ...
            if (layerInfo.type == tw.LAYERTYPE_TILES) {
                ...
                if (tileLayerInfo.image == -1) {
                    if (first) {
                        ...
                        var tiles = tw.parseLayerTiles(
                            tileData,
                            tileLayerInfo.width * tileLayerInfo.height
                        );
                ...
                if (tex == undefined) console.log("couldn't find texture", quadLayerInfo.image);
                else {
                    // load tiles
                    var tileData = dFile.getData(tileLayerInfo.data);
                    var tiles = tw.parseLayerTiles(
                ...
                var tex;
                if (quadLayerInfo.image != -1) tex = this.getTex(quadLayerInfo.image);
...
tw.Map.prototype.getTex = function (imgId) {
    for (var i = 0; i < this.textures.length; i++) {
        if (this.textures[i].imgId == imgId) return this.textures[i];
    }
};
...
tw.QuadLayer.prototype.tick = function () {
    ...
    for (var i = 0; i < this.quads.length * 4; i += 4) {
        var indices = [i, i + 1, i + 2, i, i + 2, i + 3];
...
tw.QuadLayer.prototype.render = function () {
    if (this.texture == undefined) tw.sTexCoords(0);
    //textureless quad
    else if (!this.texture.loaded) return;
    // texture not loaded
    else {
        tw.sTexCoords(1);
...
tw.Map.Group.prototype.tick = function () {
    for (var i = 0; i < this.layers.length; i++) this.layers[i].tick();
};
...
tw.Map.Group.prototype.render = function () {
    ...
    for (var i = 0; i < this.layers.length; i++) this.layers[i].render();
};

tw.Map.prototype.tick = function () {
    for (var i = 0; i < this.groups.length; i++) this.groups[i].tick();
};
...
tw.Map.prototype.render = function () {
    // Render all groups
    for (var i = 0; i < this.groups.length; i++) this.groups[i].render();
...
tw.mainLoop = function () {
    ...
    tw.mouseDownInc[0] =
        ((tw.mouseDownInc[0] / tw.canvas.width) * tw.worldView[0] * tw.aspect) / tw.cameraZoom;
    tw.mouseDownInc[1] =
        ((tw.mouseDownInc[1] / tw.canvas.height) * tw.worldView[1]) / tw.cameraZoom;
...
tw.TileLayer.prototype.initBuffers = function () {
    ...
    for (y = 0; y < this.height; y++) {
        for (x = 0; x < this.width; x++) {
            ...
            vertices = [x, y, x, y + 1.0, x + 1.0, y + 1.0, x, y, x + 1.0, y + 1.0, x + 1.0, y];
            ...
            texCoords = [x0, y0, x3, y1, x1, y2, x0, y0, x1, y2, x2, y3];
...
tw.TileLayer.prototype.render = function () {
    if (!this.texture.loaded) return; // we can't render without texture
